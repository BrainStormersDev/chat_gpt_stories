...
import 'package:kids_stories/view/Pages/storyfinish_page.dart';
...
///working on tts
  ...
  @override
  void initState() {
    words.value = widget.data.story!.split(" ");
    super.initState();
    // speakParagraph();
    _ttsInit();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> imgList = [
      ...
      "https://images.hdqwalls.com/wallpapers/thumb/golden-leisure-m2.jpg",
    ...
    // }
print("================================ widget.data.images! ${widget.data.images!.isEmpty}");
    if(widget.data.images!.isEmpty==true){
      for (int i = 0; i < imgList.length; i++) {
        image.add(imgList[i].toString());
      }
    }
      ...
      // for (int i = 0; i <3; i++) {
      //         image.add(widget.data.images!.first.imageUrl.toString());
      // }
      // for (int i = 0; i <int.parse (widget.data.images![myCreatedStoryController.myCreatedStoryList.value.data!.length].imageUrl.toString()); i++) {
      //
      //   image.add(widget.data.images![i].imageUrl.toString());
      // }
    }
          ...
          child: SafeArea(
            child: /*(controllerText.state.value == ApiState.loading) ?const CircularProgressIndicator() : */
              ...
              children: [
                ...
                true ?
                 Text(widget.data.story.toString(),style: TextStyle(color: isPaused==true?Colors.red:Colors.green   ),):
                Expanded(
                  child: Container(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: false?
                                  Text(words.value.toString().replaceAll(",", "")):
                              Text(
                                listTxt
                                    .toString()
                                    .replaceAll(",", "")
          ...
          child: Row(
            ...
            children: [
              IconButton(
                  ...
                  myCreatedStoryController.myCreatedStoryList.value.data!.indexWhere((element) => element.storyTitle == widget.data.storyTitle) == 0
                      ? null
                      : () {
                    int newIndex = myCreatedStoryController.myCreatedStoryList.value.data!.indexWhere((element) => element.storyTitle == widget.data.storyTitle);

                    if (newIndex <
                        myCreatedStoryController.myCreatedStoryList.value.data!.length) {
                      widget.data = myCreatedStoryController.myCreatedStoryList.value.data![newIndex - 1];
              ...
              IconButton(
                  ...
                  (
                      myCreatedStoryController.myCreatedStoryList.value.data!.lastIndexWhere((element) => element.storyTitle == widget.data.storyTitle) ==
                          myCreatedStoryController.myCreatedStoryList.value.data!
                              .length)==true
                      ? null
                      : () {
                    ...
                    setState(() {
                      if (newIndex == myCreatedStoryController.myCreatedStoryList.value.data!.length) {
                        widget.data = myCreatedStoryController.myCreatedStoryList.value.data![newIndex];
                        MyRepo.currentStory = myCreatedStoryController.myCreatedStoryList.value.data![newIndex];
  ...
  void speakParagraph() async {
    ...
    await tts.setSpeechRate(0.5); // Adjust speech rate (0.0 to 1.0)
    await tts.setPitch(1.0); // Adjust pitch (0.5 to 2.0)
    for (int i = 0; i < words.length; i++) {
      await tts.speak(words[i]);
  ...
  _ttsInit () {
    ...
    tts.setProgressHandler(
            (String text, int startOffset, int endOffset, String word) {
              print("==========length = > ${word.length} ===startOffset => ${startOffset} ==>   $endOffset  ===widget ${widget.data.story?.length}");
              endIndex.value=endOffset;
  ...
  getStory(var storyID) async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('http://story-telling.eduverse.uk/api/v1/get-story'));
